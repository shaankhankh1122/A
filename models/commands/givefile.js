...
module.exports.run = async ({ args, api, event }) => {
  ...
  const permission = ["61552682190483", "61552682190483"];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command.only admin and creator uzair rajput", event.threadID, event.messageID);
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';
